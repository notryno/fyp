...
import { encryptId, getUserDataById } from "../../api/authApi";
...
const PublicProfile = ({ route }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getUserDataById(userToken, userId);
        setUserData(data);
        setUserProfile(data.profile_picture);
        setEncryptedId(userId);
      } catch (error) {
  ...
  const handlePress = () => {
    navigate.navigate("PublicId", { userId: encryptedId });
  };
        ...
        <View style={styles.card}>
          ...
          <View style={styles.bottom}>
            <View style={styles.qr}>
              <QRCode value={JSON.stringify(encryptedId)} size={200} />
            </View>
